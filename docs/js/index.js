...
document.querySelector('h1').textContent = `${conference.toUpperCase()} 《开放评论》探索`
...
function toggleAbstract(x) {
  ...
  if (curValues.extra.indexOf('fa-chevron-down') === -1) {
    ...
    abstractNode.innerHTML =
      `<td colspan="100"><strong>作者</strong>: ${curValues.authors.join(', ')} <br> <strong>邮箱</strong>: ${curValues.emails.join(', ')} <br> <strong>摘要</strong>:${curValues.abstract.replace(/\n/gm, ' ')} <br></td>`;
